    ...
    <div className="space-y-6 animate-fade-in text-[#e4e4e7]">
      <div className="bg-[#121214] p-5 border border-[#27272a] shadow-lg rounded-3xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-md font-extrabold text-white">History Logs — Audit Trail</h2>
            <p className="text-xs text-[#71717a] font-light mt-0.5">Every system change with previous and new values.</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={fetchLogs} className="p-3 text-zinc-300 hover:text-white bg-[#0a0a0b] hover:bg-zinc-800 border border-[#27272a] rounded-2xl active:scale-95 transition"><RefreshCw className="w-4 h-4" /></button>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search user, action, value, reference..." className="w-full pl-10 pr-4 py-3 bg-[#121214] text-white border border-[#27272a] rounded-2xl text-xs focus:ring-2 focus:ring-blue-500 focus:outline-none transition" />
        </div>
        <select value={actionFilter} onChange={(e) => setActionFilter(e.target.value)} className="px-3 py-3 bg-[#121214] text-zinc-300 border border-[#27272a] rounded-2xl text-xs font-bold [&>option]:bg-[#121214]">
              ...
              <tbody className="divide-y divide-[#27272a]">
                {filtered.map((l) => (
                  <tr key={l.id} className="hover:bg-[#1c1c1f]/40 transition align-top">
                    <td className="p-3 font-bold text-white">{l.operator_name}</td>
                    <td className="p-3 text-zinc-400">{l.operator_role || "—"}</td>
                    ...
                    <td className="p-3 font-mono text-[10px] text-zinc-400 whitespace-nowrap">{fmt(l.timestamp)}</td>
                    <td className="p-3 text-zinc-400 max-w-[220px]">{l.details || l.related_ref || "—"}</td>
                  </tr>
                ))}
                {filtered.length === 0 && <tr><td colSpan={8} className="p-8 text-center text-zinc-500">No history entries.</td></tr>}
              </tbody>
            </table>
          </div>
          <div className="p-4 bg-[#0a0a0b] text-[11px] text-[#71717a] border-t border-[#27272a]">{filtered.length} entries</div>
        </div>
      )}
    </div>
  );
